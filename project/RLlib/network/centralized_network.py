...
class CentralizedCriticModel(RecurrentNetwork, nn.Module):
    def __init__(
        ...
        self.communication_shape = model_config["custom_model_config"].get("communication_shape", (0, 0))
        ...
        self.select_group = model_config["custom_model_config"].get("select_group", False)
    ...
    def central_value_function(self, obs, opponent_obs, opponent_actions):
        ...
        opponent_flatten_feature  = [opponent_inventory_flatten]
        for key, value in opponent_obs_restore.items():
            if key in ['final_split', 'proposal', 'available_player']:
                opponent_flatten_feature.append(value.reshape(B, -1))
                
        opponent_flatten_feature = torch.cat(opponent_flatten_feature, dim = -1)
        ...
        x = torch.cat((x, flatten_feature, opponent_flatten_feature), dim = -1)
